...
class AtomComposer(object):
    ...
    def get_knn_graph(data:ProteinLigandData, knn, len_ligand_ctx, len_compose, num_workers=1, ):
        try:
            ...
            # Fallback for environments where torch-cluster is not available.
            pos = data.compose_pos
            num_nodes = pos.size(0)
            k = min(knn, max(num_nodes - 1, 0))
            if k == 0:
                data.compose_knn_edge_index = torch.empty((2, 0), dtype=torch.long, device=pos.device)
            else:
                pair_dist = torch.cdist(pos, pos)
                pair_dist.fill_diagonal_(float('inf'))
                source = pair_dist.topk(k, largest=False, dim=1).indices.reshape(-1)
                target = torch.arange(num_nodes, device=pos.device).unsqueeze(1).expand(num_nodes, k).reshape(-1)
                data.compose_knn_edge_index = torch.stack([target, source], dim=0)
