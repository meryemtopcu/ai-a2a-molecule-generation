...
class MaskFillModelVN(Module):
    ...
    def get_tri_edges(self, edge_index_query, pos_query, idx_ligand, ligand_bond_index, ligand_bond_type):
        ...
        for node in torch.arange(pos_query.size(0)):
            ...
            index_edge_i = torch.arange(num_edges, dtype=torch.long, ).to('cpu') + acc_num_edges
        ...
        n_context = len(idx_ligand)
        adj_mat = (torch.zeros([n_context, n_context], dtype=torch.long) - torch.eye(n_context, dtype=torch.long)).to('cpu')
        ...
        tri_edge_feat = (tri_edge_type.view([-1, 1]) == torch.tensor([[-1, 0, 1, 2, 3]]).to('cpu')).long()
